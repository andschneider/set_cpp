...
std::vector<std::vector<Point> > diamond_contour = LoadContour("./images/diamond2.png");
std::vector<std::vector<Point> > squiggle_contour = LoadContour("./images/squiggle.png");
std::vector<std::vector<Point> > oval_contour = LoadContour("./images/oval2.png");
...
class Card {
 ...
 public:
  ...
  std::vector<char> GetClassification() {
    // TODO add these to the classification_ class variable when they are calculated
    return std::vector<char>{(char)shape_id_, (char)color_id_, (char)num_symbol_,
                             (char)shading_id_};
  }
  std::vector<std::string> GetCardInfo() {
    return std::vector<std::string>{shape_, color_, std::to_string(num_symbol_ + 1),
                                    shading_};
  }
  int DetermineColor();
  int DetermineNumber();
...
int Card::DetermineColor() {
    // green
    inRange(hsv_image_, Scalar(60, 100, 100), Scalar(80, 255, 255), thresh_image_);
    ...
    inRange(hsv_image_, Scalar(110, 50, 100), Scalar(130, 150, 255), thresh_image_);
    if (countNonZero(thresh_image_) > 0) {
      color_id_ = 2;
      color_ = colors_[2];
      return color_id_;
...
// Finds the shading_ of the symbol. Either outlined (0), solid (1), or striped (2).
int Card::DetermineShading() {
  // Flood fill contour
  std::vector<Vec4i> hierarchy;
  Mat filled = Mat::zeros(thresh_image_.size(), CV_8UC1);
  drawContours(filled, contours_, 0, CV_RGB(255, 255, 255), -1, 8, hierarchy, 0);
